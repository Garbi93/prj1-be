...
public class MemberService {
    ...
    public boolean deleteMember(String id) {
        // 이 맴버가 작성한 댓글 삭제
        commentMapper.deletByMemberId(id);

        // 1. 이 맴버가 작성한 게시물 삭제 -> boardMapper를 주입 받는다

        // 1-1 이 맴버가 작성한 게시물 번호들 조회
        ...
        // 2 이 맴버가 작성한 좋아요 번호 조회
        List<Integer> boardLikeId = likeMapper.selectIdLikeByMemberId(id);
        // 2-2 좋아요 번호들 loop 각 좋아요 삭제
        boardLikeId.forEach((likeId)->likeMapper.deleteById(likeId));
